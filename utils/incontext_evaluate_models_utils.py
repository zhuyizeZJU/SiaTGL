...
from utils.metrics import get_link_prediction_metrics, get_node_classification_metrics,get_link_prediction_metrics_original
...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000,args=None,minn_ls = None,maxx_ls=None):
    """
    ...
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer','DyGFormer','FreeDyG','SiaTGL']:
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch_data in enumerate(evaluate_idx_data_loader_tqdm):
            batch_src_node_ids = batch_data[0].astype(int)[0]
            batch_dst_node_ids = batch_data[1].astype(int)[0]
            ...
            if model_name in ['DyGFormer']:
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      dst_node_ids=batch_dst_node_ids,
                                                                      node_interact_times=batch_node_interact_times,
                                                                      node_sampleInterval=batch_sampleInterval)
            ...
            evaluate_metrics.append(get_link_prediction_metrics_original(predicts=predicts, labels=labels.float()))
...
def evaluate_model_node_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    :return:
    """
    if model_name in ['DyRep', 'TGAT', 'TGN', 'CAWN', 'TCL', 'GraphMixer', 'DyGFormer','DyCoN','DyCoD']:
        # evaluation phase use all the graph information
        model[0].set_neighbor_sampler(neighbor_sampler)
...
def evaluate_edge_bank_link_prediction(args: argparse.Namespace, train_data: Data, val_data: Data, test_idx_data_loader: DataLoader,
                                       test_neg_edge_sampler: NegativeEdgeSampler, test_data: Data,trainInterval = '0',testInterval = str):
    """
    ...
    for run in range(args.num_runs):
        ...
        for batch_idx, test_data_indices in enumerate(test_idx_data_loader_tqdm):
            ...
            test_metrics.append(get_link_prediction_metrics_original(predicts=predicts, labels=labels))

            test_idx_data_loader_tqdm.set_description(f'test for the {batch_idx + 1}-th batch, test loss: {loss.item()}')

        test_metric_dict = {}
    avg_precisions = [test_metric["average_precision"] for test_metric in test_metrics]
    average_precision_mean = np.mean(avg_precisions)
    print(average_precision_mean)
            
def evaluate_model_link_prediction_sampleIntervalAware(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluate_data: Data, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000,args=None,minn_ls = None,maxx_ls=None):
    """
    evaluate models on the link prediction task
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch_data in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            batch_neg_dst_node_ids = batch_data[3].astype(int)[0]
            batch_node_interact_times = batch_data[4].astype(float)[0]
            batch_sampleInterval = batch_data[5].astype(float)[0]
            batch_edge_ids = batch_data[6].astype(float)[0]
            ...
            if model_name in ['TGAT', 'CAWN', 'TCL']:
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      ...
                                                                      edges_are_positive=True,
                                                                      num_neighbors=num_neighbors)
            elif model_name in ['GraphMixer','NeighborSeq']:
                # get temporal embedding of source and destination nodes
                # two Tensors, with shape (batch_size, node_feat_dim)
                batch_src_node_embeddings, batch_dst_node_embeddings = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                ...
                batch_src_node_embeddings, batch_dst_node_embeddings,_ = \
                    model[0].compute_src_dst_node_temporal_embeddings(src_node_ids=batch_src_node_ids,
                                                                      ...
                                                                      node_interact_times=batch_node_interact_times,
                                                                      node_sampleInterval=batch_sampleInterval)
                                                                    #   node_sampleInterval=batch_sampleInterval,isTrain=False)
...
def evaluate_model_link_prediction_sampleIntervalAware_mixed(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   evaluate_neg_edge_sampler: NegativeEdgeSampler, evaluate_data: Data, loss_func: nn.Module,
                                   num_neighbors: int = 20, time_gap: int = 2000,args=None,minn_ls = None,maxx_ls=None):
    """
    evaluate models on the link prediction task
