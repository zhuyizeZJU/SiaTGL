...
from utils.DataLoader import Data,Data_withSampleInterval
...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        self.nodes_neighbor_sampleInterval = []
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            if len(per_node_neighbors) > 0 and len(per_node_neighbors[0]) > 3:
                self.nodes_neighbor_sampleInterval.append(np.array([x[3] for x in sorted_per_node_neighbors]))
            else:
                # Append an empty array or handle the case accordingly
                self.nodes_neighbor_sampleInterval.append(np.array([]))
            # self.nodes_neighbor_sampleInterval.append(np.array([x[3] for x in sorted_per_node_neighbors]))
    ...
    def find_neighbors_before_sampleInterval(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        ...
        theSampleInterval = self.nodes_neighbor_sampleInterval[node_id]
        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_id][:i], self.nodes_edge_ids[node_id][:i], self.nodes_neighbor_times[node_id][:i], \
                   self.nodes_neighbor_sampled_probabilities[node_id][:i], self.nodes_neighbor_sampleInterval[node_id][:i]
        else:
            return self.nodes_neighbor_ids[node_id][:i], self.nodes_edge_ids[node_id][:i], self.nodes_neighbor_times[node_id][:i], None, self.nodes_neighbor_sampleInterval[node_id][:i]
    ...
    def find_neighbors_leq(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        extracts all the interactions happening before or at interact_time (less than or equal to interact_time) 
        for node_id in the overall interaction graph.
        The returned interactions are sorted by time.
        
        :param node_id: int, node id
        :param interact_time: float, interaction time
        :param return_sampled_probabilities: boolean, whether to return the sampled probabilities of neighbors
        :return: neighbors, edge_ids, timestamps and sampled_probabilities (if return_sampled_probabilities is True) 
        with shape (historical_nodes_num, )
        """
        # Adjusting the search to find the position where interact_time could fit
    ...
    def find_neighbors_lt_sampleInterval(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        extracts all the interactions happening strictly before interact_time (less than interact_time) 
        for node_id in the overall interaction graph.
        The returned interactions are sorted by time.
        ...
        # Adjusting the search to find the position where interact_time could fit
        i = np.searchsorted(self.nodes_neighbor_times[node_id], interact_time, side='left')

        if return_sampled_probabilities:
            return (self.nodes_neighbor_ids[node_id][:i], 
                    self.nodes_edge_ids[node_id][:i], 
                    self.nodes_neighbor_times[node_id][:i], 
                    self.nodes_neighbor_sampleInterval[node_id][:i], 
                    self.nodes_neighbor_sampled_probabilities[node_id][:i])
        else:
            return (self.nodes_neighbor_ids[node_id][:i], 
                    self.nodes_edge_ids[node_id][:i], 
                    ...
                    self.nodes_neighbor_sampleInterval[node_id][:i], 
                    None)
    def find_neighbors_leq_sampleInterval(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        """
        extracts all the interactions happening before or at interact_time (less than or equal to interact_time) 
        for node_id in the overall interaction graph.
        The returned interactions are sorted by time.
    ...
    def get_historical_neighbors_sampleInterval(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        """
        get historical neighbors of nodes in node_ids with interactions before the corresponding time in node_interact_times
        :param node_ids: ndarray, shape (batch_size, ) or (*, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ) or (*, ), node interaction times
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_neighbor_times = np.zeros((len(node_ids), num_neighbors)).astype(np.float32)
        nodes_neighbor_sampleInterval = np.zeros((len(node_ids), num_neighbors)).astype(np.float32)

        # extracts all neighbors ids, edge ids and interaction times of nodes in node_ids, which happened before the corresponding time in node_interact_times
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            # find neighbors that interacted with node_id before time node_interact_time
            node_neighbor_ids, node_edge_ids, node_neighbor_times, node_neighbor_sampled_probabilities,node_neighbor_sampleInterval = \
                self.find_neighbors_before_sampleInterval(node_id=node_id, interact_time=node_interact_time, return_sampled_probabilities=self.sample_neighbor_strategy == 'time_interval_aware')
            ...
            if len(node_neighbor_ids) > 0:
                if self.sample_neighbor_strategy in ['uniform', 'time_interval_aware']:
                    ...
                    nodes_edge_ids[idx, :] = node_edge_ids[sampled_indices]
                    nodes_neighbor_times[idx, :] = node_neighbor_times[sampled_indices]
                    nodes_neighbor_sampleInterval[idx, :] = node_neighbor_sampleInterval[sampled_indices]
                    ...
                    nodes_edge_ids[idx, :] = nodes_edge_ids[idx, :][sorted_position]
                    nodes_neighbor_times[idx, :] = nodes_neighbor_times[idx, :][sorted_position]
                    node_neighbor_sampleInterval[idx, :] = node_neighbor_sampleInterval[idx, :][sorted_position]
                elif self.sample_neighbor_strategy == 'recent':
                    # Take most recent interactions with number num_neighbors
                    node_neighbor_ids = node_neighbor_ids[-num_neighbors:]
                    node_edge_ids = node_edge_ids[-num_neighbors:]
                    node_neighbor_times = node_neighbor_times[-num_neighbors:]
                    node_neighbor_sampleInterval = node_neighbor_sampleInterval[-num_neighbors:]

                    # put the neighbors' information at the back positions
                    nodes_neighbor_ids[idx, num_neighbors - len(node_neighbor_ids):] = node_neighbor_ids
                    nodes_edge_ids[idx, num_neighbors - len(node_edge_ids):] = node_edge_ids
                    nodes_neighbor_times[idx, num_neighbors - len(node_neighbor_times):] = node_neighbor_times
                    nodes_neighbor_sampleInterval[idx, num_neighbors - len(node_neighbor_sampleInterval):] = node_neighbor_sampleInterval
                else:
                    raise ValueError(f'Not implemented error for sample_neighbor_strategy {self.sample_neighbor_strategy}!')

        # three ndarrays, with shape (batch_size, num_neighbors)
        return nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times, nodes_neighbor_sampleInterval
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        ...
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            node_neighbor_ids, node_edge_ids, node_neighbor_times, _ = self.find_neighbors_leq(node_id=node_id,
    ...
    def get_all_first_hop_neighbors_sampleInterval(self, node_ids: np.ndarray, node_interact_times: np.ndarray):
        """
        get historical neighbors of nodes in node_ids at the first hop with max_num_neighbors as the maximal number of neighbors (make the computation feasible)
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        ...
        # three lists to store the first-hop neighbor ids, edge ids and interaction timestamp information, with batch_size as the list length
        nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list,nodes_neighbor_sampleInterval_list = [], [], [],[]
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            node_neighbor_ids, node_edge_ids, node_neighbor_times, nodes_neighbor_sampleInterval,_ = self.find_neighbors_lt_sampleInterval(node_id=node_id,
                                                                                                  interact_time=node_interact_time,
                                                                                                  return_sampled_probabilities=False)
            nodes_neighbor_ids_list.append(node_neighbor_ids)
            nodes_edge_ids_list.append(node_edge_ids)
            nodes_neighbor_times_list.append(node_neighbor_times)
            nodes_neighbor_sampleInterval_list.append(nodes_neighbor_sampleInterval)

        return nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list,nodes_neighbor_sampleInterval_list
...
def get_neighbor_sampler_sampleInterval(data: Data_withSampleInterval, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    """
    get neighbor sampler
    :param data: Data
    :param sample_neighbor_strategy: str, how to sample historical neighbors, 'uniform', 'recent', or 'time_interval_aware''
    ...
    # the list at the first position in adj_list is empty
    adj_list = [[] for _ in range(max_node_id + 1)]
    for src_node_id, dst_node_id, edge_id, node_interact_time,sampleInterval in zip(data.src_node_ids, data.dst_node_ids, data.edge_ids, data.node_interact_times,data.sampleInterval):
        adj_list[src_node_id].append((dst_node_id, edge_id, node_interact_time,sampleInterval))
        adj_list[dst_node_id].append((src_node_id, edge_id, node_interact_time,sampleInterval))
